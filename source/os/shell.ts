    ...
    export class Shell {
        ...
        public init() {
            ...
            sc = new ShellCommand(this.shellGetSchedule,
                                    "getschedule",
                                    "- Returns the type of scheduling algorithm being used");
            this.commandList[this.commandList.length] = sc;
            _AllCommands.push(sc.command);
            ...
            sc = new ShellCommand(this.shellSetSchedule,
                                        "setschedule",
                                        "- Set the type of scheduling algorithm being used");
            this.commandList[this.commandList.length] = sc;
            _AllCommands.push(sc.command);            
           ...
           if(inputMatch.value.match('[^A-F0-9\\s]') || inputMatch.value.length == 0) {
                _StdOut.putText("Invalid Input");
                    ...
                    if(_MemoryManager.memoryFull() == true){
                        // Check if the Hard Drive is formatted so we can load processes to memory
